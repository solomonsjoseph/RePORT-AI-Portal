...
etc.) and paraphrases them as a name / variable answer — a poor UX
even though the PHI safety gates fire correctly.
...
``@tool``-wrapped search functions via their underlying callable
...
class TestQueryLooksConversational:
    """The predicate the surviving search tools use to short-circuit."""
    ...
    def test_real_research_queries_pass_through(self) -> None:
        ...
        assert _query_looks_conversational("tuberculosis treatment outcomes") is False
...
class TestAnswerCatalogQuestionShortCircuit:
    """answer_catalog_question refuses conversational inputs."""

    def test_hi_returns_refusal_message(self) -> None:
        from scripts.ai_assistant.agent_tools import answer_catalog_question

        fn: Any = getattr(answer_catalog_question, "func", answer_catalog_question)
        out = fn("hi")
        assert out == _CONVERSATIONAL_REFUSAL_MESSAGE
