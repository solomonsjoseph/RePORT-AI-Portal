...
Happy path
----------
One test builds a complete golden output tree (satisfying all 15 assertions)
...
def _patch_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    # tmp/data/raw so study_config_path resolves the manifest that
    # build_golden_output_tree writes under raw_root/<study>/.
