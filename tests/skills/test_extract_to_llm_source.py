...
# ---------------------------------------------------------------------------
# PHI coverage-hold human-review note (W0 Piece B)
# ---------------------------------------------------------------------------


class TestCoverageReviewNotes:
    """A form held for a PHI coverage concern gets a metadata-only review note."""

    @staticmethod
    def _approval(form_name: str, reasons: tuple[str, ...]):
        from types import SimpleNamespace

        return SimpleNamespace(form_name=form_name, reasons=list(reasons))

    def test_note_written_for_coverage_held_form(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        import config
        from scripts.skills.extract_to_llm_source import _write_coverage_review_notes

        monkeypatch.setattr(config, "OUTPUT_DIR", tmp_path)
        approvals = [
            self._approval(
                "04_FollowUp.xlsx",
                ("phi_coverage_hold: 'interviewer_remarks' is KEEP but ...",),
            ),
            self._approval("01_Enroll.xlsx", ()),  # clean form — no note
        ]

        noted = _write_coverage_review_notes("Indo-VAP", approvals)

        assert noted == ["04_FollowUp.xlsx"]
        note_path = (
            tmp_path
            / "Indo-VAP"
            / "audit"
            / "human_review"
            / "04_FollowUp"
            / "phi_coverage_review.md"
        )
        assert note_path.is_file()
        body = note_path.read_text(encoding="utf-8")
        assert "interviewer_remarks" in body
        assert "held_publish_review" in body
        # clean form must NOT get a note
        assert not (tmp_path / "Indo-VAP" / "audit" / "human_review" / "01_Enroll").exists()

    def test_note_lives_in_audit_no_llm_zone(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Note path must be under output/{study}/audit/ (denied to the agent)."""
        import config
        from scripts.skills.extract_to_llm_source import _write_coverage_review_notes

        monkeypatch.setattr(config, "OUTPUT_DIR", tmp_path)
        _write_coverage_review_notes(
            "Indo-VAP",
            [self._approval("x.xlsx", ("phi_coverage_hold: 'name' is KEEP ...",))],
        )
        note = tmp_path / "Indo-VAP" / "audit" / "human_review" / "x" / "phi_coverage_review.md"
        assert "audit" in note.parts
        assert note.is_file()
