...
class TestEmitLineageManifest:
    def test_full_manifest(
        ...
        (trio / "datasets" / "scrubbed.jsonl").write_bytes(b"clean-rows")
        (trio / "variables.json").write_bytes(b"{}")
        ...
        # Outputs enumerated
        trio_paths = {r["path"] for r in on_disk["outputs"]["trio_bundle"]}
        assert "datasets/scrubbed.jsonl" in trio_paths
        assert "variables.json" in trio_paths
        # Step metadata
