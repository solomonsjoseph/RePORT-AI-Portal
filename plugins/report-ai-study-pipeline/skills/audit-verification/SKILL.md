...
description: Run the 14-assertion post-publish verifier for a completed run — manifest reconciliation, ledger hashes + no-LLM sentinel, quarantine-empty, PHI absence scan, decided-vs-applied protection lattice, and ledger coverage completeness — with the canonical exit-code routing. Exit codes only, never row values.
---
...
exit-code routing. The 14 assertions (executed 1→12→14→13) cover:
...
| Ledger coverage completeness | 14 | 10 |
